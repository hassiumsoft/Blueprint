...
public class Chunk : ISerializable {
	...
	//フラクタル地形の細かさ（細分化回数）
	//2017年6月時点での生成速度：3では20秒程度かかった。 4では1分程度かかった。 5では7分~28分程度かかった。 6では数十分~数時間以上の時間がかかった。
	//7ではメッシュの頂点数が制限(65000)を超えてしまうので不可能。
	//描画を優先しない場合はfinenessが3で4/1秒。 4では2~3秒程度。
	//スペックによる差があるため3倍かかると見込んだほうが良い。
	//理想の細かさは6~7 (size/2^fineness=1になる数値)
	//
	//2017年10月時点では1チャンク1~2秒（非同期で1.5秒）
	//
